...
for w = 1:n
    ...
    uh = dg_rk3_scheme_with_limiter(uh0, dx, t1, @burgers_f, @burgers_fhat_LF, @burgers_df, pk, gk, basis, basis_dx, limiter);
...
for w = 1:2
    ...
    init_func = @(s) alpha + beta * sin(s);
    uh0 = dg_projection(init_func, x, dx, pk, gk, basis);
    uh = dg_rk3_scheme_with_limiter(uh0, dx, t2, @burgers_f, @burgers_fhat_LF, @burgers_df, pk, gk, basis, basis_dx, limiter);
