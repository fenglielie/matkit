classdef Logger < handle
    ...
    %
    % Example usage:
    %   logger = Logger(fileName='tmp.log', level=Logger.DEBUG, outputToConsole=true);
    ...
    %
    % Properties:
    %   level - The minimum level of log messages to display, default: INFO
    ...
    methods
        function obj = Logger(varargin)
            ...
            %   - level=Logger.INFO : Minimum log level (1=DEBUG, 2=INFO, 3=WARN, 4=ERROR).
            ...
            %   - format="level" : Log format: "none", "level", "timestamp_and_level".
            ...
            %   logger = Logger(level=Logger.WARN, outputToConsole=false);  % Logs warnings/errors to file only
            %   logger = Logger(format="timestamp_and_level");  % Includes timestamps in log messages
            ...
            addParameter(p, 'format', 'level', @(x) ischar(x) && any(strcmp(x, {'none', 'level', 'timestamp_and_level'})));
